...
const CommentList = ({ workoutId, token, refreshComments, onEditComment }) => {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            try {
                const res = await axios.get(`https://fitnessworkouttracker-1.onrender.com/api/comments/${workoutId}`, {
    ...
    const handleDeleteComment = async (commentId) => {
        try {
            await axios.delete(`https://fitnessworkouttracker-1.onrender.com/api/comments/${commentId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
